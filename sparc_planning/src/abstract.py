...
from example_domains.example_5bladder import generate_domain_setup
...
FINE_ITERATION_LIMIT = 32
COARSE_ITERATION_MIN = 0
COARSE_PLAN_ITERATION_LIMIT = 40
LOCATION_REDUCTION = True
...
async def main():
    ...
    for i in range(5):
        ...
        assem = load_assembly_xml(beams, os.path.join(os.environ['PLANNER_PATH'], "assembly_5beamladder.xml"))
        logging.info(assem.beams)
        ...
        for step in abstract_actions:
            ...
            # fudge added to limit size of answer sets as more pins are added for ladder frames only
            if inserted_beam != 'b8':
                coarse_goals.append([GoalDefinition('fastened_c',[inserted_beam,'b7',f'p{pin_num}'],True)])
                pin_num+=1
        fastening_goal = []
        for i in range(len(assem.connections)):
            ...
            fastening_goal.append(GoalDefinition('can_fasten_c',[B1,B2],False))
        for fg in fastening_goal:
            coarse_goals.append([fg])

        #logging.info(coarse_goals)
        ...
        try:
            for i in range(len(all_coarse_actions)):
                ...
                zoomed_fine_res_description = zoom(all_coarse_states[i], all_coarse_states[i+1], actr, coarse, fine, LOCATION_REDUCTION)
           ...
           # logging.info(f'Abstract Actions:\n {abstract_actions}\n\n')
            atimes.append(f'{t_stop_abs-t_start_abs:.03f}')
            ...
            ctimes.append(f'{t_stop_coarse-t_start_coarse:.03f}')
            if fine_plan_success:
                ...
                ftimes.append(f'{t_stop_fine-t_start_fine:.03f}')
            else:
